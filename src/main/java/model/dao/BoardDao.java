...
public class BoardDao {
	...
	// Board board getBoardOne - m1ModifyBoardForm.jsp
	public Board selectBoardOne(Connection conn, int no) throws Exception {
		Board board = null;
		String sql = "SELECT no, title, content FROM board WHERE no = ?"; //ORDER BY 없다면 no, 인덱스키에 의해 자동정렬
		PreparedStatement stmt = conn.prepareStatement(sql);
		stmt.setInt(1, no);
		ResultSet rs = stmt.executeQuery();
		//받아오는 값이 하나니까 ArrayList보단 Board로 
		if(rs.next()) {
			board = new Board();
